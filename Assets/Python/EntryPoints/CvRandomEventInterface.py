...
# Natural Mayhem Mod - (c) October 2012 by Lemon Merchant
#
# This mod shows weather effects from some of the Random Events in BTS, and it 
# is an extension of Smeagolheart's Weather Mod from his Destiny Modpack.
# This mod has been converted to a BUG compatible module and will not run without 
# BUG or BAT installed.
# Items changed are marked by "# Mayhem"
...
### This function is called by Hurricane, Monsoon, Cyclone, etc.  The effect is the same		
def applyHurricane1(argsList):
	...
	# Do the Hurricane graphics for coastal cities
	if city.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
		# The graphic cannot appear over the city - looks bad.  Do graphic on adjacent plot that is ocean
		
		plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
		iCoastalType = CvUtil.findInfoTypeNum(gc.getTerrainInfo,gc.getNumTerrainInfos(),'TERRAIN_COAST')
		# iCoastalType = plot.getTerrainType(gc.getInfoTypeForString('TERRAIN_COAST'))
		bPlotSelected = false
		
		while bPlotSelected == false:
			for iDX in range(-1, 2):
				for iDY in range(-1, 2):
					loopPlot = plotXY(kTriggeredData.iPlotX, kTriggeredData.iPlotY, iDX, iDY)
					### This loop should break after the first time it finds an adjacent coastal tile to the city
					if not loopPlot.isNone() and bPlotSelected == false:
						if ((iDX != 0 or iDY != 0) and (loopPlot.isOwned()) and plot.getTerrainType() == iCoastalType):
							# newPlot = gc.getMap().plot(iDX, iDY)
							plot = loopPlot
							bPlotSelected = true
							# need to break the for loops too?
							break
							
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_HURRICANE'), 0)
	
	## If the city is inland and qualifies for the Monsoon event
	elif not city.isCoastal(gc.getMIN_WATER_SIZE_FOR_OCEAN()):
		# plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
		listPlots = []
		for iDX in range(-1, 2):
			for iDY in range(-1, 2):
		...
		if len(listPlots) > 0:
			plot = listPlots[gc.getGame().getSorenRandNum(len(listPlots), "Monsoon event graphic placement")]
...
def applyTsunami1(argsList):
	...
	plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	#plot.setFeatureType(gc.getInfoTypeForString('FEATURE_TSUNAMI_EAST'), 0)

	if ((plot.getX() < city.getCityPlot().getX()) and (plot.getY() == city.getCityPlot().getY())):
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_TSUNAMI_EAST'), 0)
		
	if ((plot.getX() > city.getCityPlot().getX()) and (plot.getY() == city.getCityPlot().getY())):
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_TSUNAMI_WEST'), 0)
		
	if ((plot.getX() == city.getCityPlot().getX()) and (plot.getY() < city.getCityPlot().getY())):
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_TSUNAMI_SOUTH'), 0)
		
	if ((plot.getX() == city.getCityPlot().getX()) and (plot.getY() > city.getCityPlot().getY())):
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_TSUNAMI_NORTH'), 0)	
...
def doImpactCrater2(argsList):
	...
	if not plot.isNone():
		# Set the meteor strike art - play the nif files - Meteor, explosion, then set mine
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_METEOR'), 0)
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_METEOR_HIT'), 0)
		# Remove the meteor strike
		plot.setFeatureType(-1, 0)
		# Now set the mine improvement type as normal.
...
def doCometFragment(argsList):
	...
	if not plot.isNone():
		# Set the meteor strike art - play the nif files - Meteor, explosion, then set the Crater feature
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_METEOR'), 0)
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_METEOR_HIT'), 0)
		# Remove the meteor strike
		plot.setFeatureType(-1, 0)
		# Now set the mine improvement type as normal.
		plot.setFeatureType(gc.getInfoTypeForString('FEATURE_CRATER'), 0)
...
def applyForestfire1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	
	plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	plot.setFeatureType(gc.getInfoTypeForString('FEATURE_FORESTFIRE_AFTERMATH'), 0)
	
######## DUST BOWL ###########

def applyDustBowl1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	
	plot = gc.getMap().plot(kTriggeredData.iPlotX, kTriggeredData.iPlotY)
	plot.setFeatureType(gc.getInfoTypeForString('FEATURE_DUSTBOWL'), 0)	

	
######## Remove Features! ###########
	
def expireFeature1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
